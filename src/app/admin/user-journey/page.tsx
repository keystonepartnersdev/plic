...
import {
  RefreshCw, Eye, MousePointer, Clock, LogOut, Monitor,
  Smartphone, Tablet, TrendingUp, AlertTriangle, ArrowDown,
  Users, Activity, Globe, ChevronRight,
} from 'lucide-react';
...
export default function UserJourneyPage() {
  ...
  const tabs = [
    ...
    { key: 'events' as const, label: '최근 이벤트' },
  ];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">유저 여정 트래킹</h1>
          <p className="text-gray-500 text-sm">미가입자 포함 모든 유저의 활동을 추적합니다</p>
        </div>
        <button onClick={fetchData} className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg border hover:bg-gray-50">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">이벤트 타입 분포</h3>
                ...
                {[
                  ...
                  return (
                    <div key={key} className="text-center">
                      <DIcon className="w-8 h-8 mx-auto mb-2 text-gray-400" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">📄 페이지별 방문 수 (Top 20)</h3>
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {data.topPages.map((p, i) => (
                  <div key={p.path} className="flex justify-between items-center py-1">
                    ...
                    <span className="text-sm font-semibold ml-2">{p.count}</span>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">🚪 이탈 페이지 Top 10</h3>
              <div className="space-y-2">
                {data.topExitPages.map((p, i) => (
                  <div key={p.path} className="flex justify-between items-center py-1">
                    ...
                    <span className="text-sm font-semibold text-red-600 ml-2">{p.count}</span>
                  </div>
                ))}
          ...
          {Object.keys(data.utmSources).length > 0 && (
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">🔗 UTM 유입 소스</h3>
          ...
          {data.dailyTrends.length > 0 && (
            <div className="bg-white rounded-xl p-5 shadow-sm">
              ...
              <div className="overflow-x-auto">
                <div className="flex gap-1 items-end h-40 min-w-[600px]">
      ...
      {activeTab === 'funnel' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FunnelChart title="📝 가입 퍼널" steps={data.signupFunnel} color="bg-blue-400" />
          <FunnelChart
            title="🔐 로그인"
            steps={data.loginFunnel}
            color="bg-green-400"
          />
      ...
      {activeTab === 'behavior' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">⏱️ 체류 시간 마일스톤</h3>
              <div className="space-y-4">
                ...
                {Object.keys(data.sessionMilestones).length === 0 && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">👁️ 랜딩 섹션 노출</h3>
              <div className="space-y-3">
                {Object.entries(data.sectionViews)
                  .sort((a, b) => b[1] - a[1])
                  .map(([section, count]) => (
                ...
                {Object.keys(data.sectionViews).length === 0 && (
                  <p className="text-gray-400 text-sm">데이터 없음</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">🖱️ CTA 클릭 현황</h3>
              <div className="space-y-3">
          ...
          {data.topErrors.length > 0 && (
            <div className="bg-white rounded-xl p-5 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">⚠️ 에러 Top 10</h3>
              <div className="space-y-2">
      ...
      {activeTab === 'events' && (
        <div className="bg-white rounded-xl p-5 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-4">🕐 최근 이벤트 (50건)</h3>
          <div className="overflow-x-auto">
                ...
                {data.recentEvents.map((e, i) => (
                  <tr key={i} className="border-b hover:bg-gray-50">
                    <td className="py-2 px-2 text-gray-400 whitespace-nowrap text-xs">
                      {((e.timestamp as string) || '').slice(11, 19)}
                    </td>

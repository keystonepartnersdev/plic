  ...
  return new Promise((resolve, reject) => {
    const req = (0, import_https.request)(uri, {
      ...
      if (response2.statusCode !== 200) {
        done(new NonRetryableFetchError(uri, `Status code is ${response2.statusCode}, expected 200`));
        return;
      }
      (0, import_stream.pipeline)(response2, async (responseBody) => {
        const chunks = [];
...
var nodeWebCompat = {
  ...
  transformPemToJwk: async (pem) => {
    return (0, import_crypto.createPublicKey)({
      key: Buffer.from(pem),
      format: "pem"
    }).export({
      format: "jwk"
...
var PAYMENTS_TABLE = process.env.PAYMENTS_TABLE || "plic-payments";
...
var getUserIdFromToken = async (authHeader) => {
...
var generatePaymentId = () => `PAY${Date.now()}${Math.random().toString(36).substr(2, 6)}`;

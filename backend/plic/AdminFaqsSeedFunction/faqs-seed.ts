// backend/functions/admin/faqs-seed.ts
  ...
  try {
    ...
    for (const faq of FAQ_DATA) {
      ...
      const item = {
        ...
        isHomeFeatured: faq.isHomeFeatured,
      ...
      results.push({ faqId, question: faq.question, category: faq.category });
    ...
    return response(200, {
      success: true,
      data: {
        message: `${results.length}개의 FAQ가 등록되었습니다.`,
        deletedCount: existingResult.Items?.length || 0,
    ...
    console.error('FAQ Seed 오류:', error);

    return response(500, {
      success: false,
      error: error.message || 'FAQ Seed 처리 중 오류가 발생했습니다.',
    });
  }
};

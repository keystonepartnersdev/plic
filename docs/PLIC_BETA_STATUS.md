...
| ~~전체 유저 여정 트래킹~~ | 2026-03-21 | ✅ 미가입/가입자 전체 추적: pageview(자동), 스크롤 깊이(25/50/75/100%), 체류시간(30s~10m), 탭전환, 이탈감지, CTA 클릭(data-track), 섹션 노출(data-section), 가입 퍼널(5단계), 로그인 퍼널(시도/성공/실패), 송금 퍼널(6단계), 결제 퍼널(진입/시도/성공/실패), UTM 파라미터, 익명ID 기반 미가입자 여정 추적 |
